...
var system;
// const { image as Image } = require('./image');
...
function device(_system, panel) {
	...
	system.on('device_inc_page', function (deviceid) {
		if (self.panel.serialnumber == deviceid) {
			self.page++;
			if (self.page == 100) { self.page = 1; }
			if (self.page == 0) { self.page = 99; }
	...
	system.on('device_dec_page', function (deviceid) {
		if (self.panel.serialnumber == deviceid) {
			self.page--;
			if (self.page == 100) { self.page = 1; }
			if (self.page == 0) { self.page = 99; }
	...
	self.on_elgato_click = function(devicepath, key, state, obj) {
		...
		if (!self.lockedout) {
			...
			system.emit('log', 'device('+self.panel.serialnumber+')', 'debug', 'Button '+self.page+'.' + (parseInt(key) + 1) + ' ' + (state ? 'pressed' : 'released'));
		}
