...
class SurfaceDriverCommon {
	...
	draw(key, rawBuffer) {
		const buffer = SurfaceDriverCommon.prepareButtonBuffer(rawBuffer, this.config.rotation);
		const drawKey = this.toDeviceKey(key);

		this.drawInner(drawKey, buffer, 0);
	}

	drawInner(drawKey, buffer, attempts) {
		attempts++;
		...
		if (drawKey !== undefined && drawKey >= 0 && drawKey < this.keysTotal) {
			try {
				...
				setTimeout(() => this.drawInner(drawKey, buffer, attempts), 20);
			}
		}
	}
	...
	static fromDeviceMap (map, key) {
		return map.indexOf(key);
	}

	static prepareButtonBuffer(buffer, rotation) {
...
module.exports = SurfaceDriverCommon;
