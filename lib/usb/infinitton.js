...
class SurfaceDriverInfinitton extends SurfaceDriverCommon {
	...
	openDevice() {
		...
		this.device.on('down', (key) => this.keyDown(SurfaceDriverInfinitton.fromDeviceMap(KEY_MAP, key)));
		this.device.on('up', (key) => this.keyUp(SurfaceDriverInfinitton.fromDeviceMap(KEY_MAP, key)));
	...
	clearKey(key) {
		if (this.device) {
			const deviceKey = SurfaceDriverInfinitton.toDeviceMap(KEY_MAP, key);
