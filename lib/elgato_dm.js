...
class PanelDeviceManager {
	...
	clientConnect(socket) {
		...
		socket.on('device_config_get', function (targetId) {
		...
		socket.on('device_config_set', function (targetId, config) {
			for (const id in this.instances) {
				if (this.instances[id].id == targetId) {
	...
	addDevice(device, type) {
		...
		if (type === 'elgatoEmulator') {
			...
			this.instances[device.path] = new usb(this.system, type, device.path, function () {
...
// Simple singleton
var deviceManagerInstance;
module.exports = function (system) {
	if (deviceManagerInstance === undefined) {
		deviceManagerInstance = new PanelDeviceManager(system);
	}
