...
satellite.prototype.buttonClear = function(key) {
	var self = this;
	debug('satellite.prototype.buttonClear('+key+')')
	self.clearKey(key);
}

satellite.prototype.isPressed = function(key) {
	var self = this;
	debug('satellite.prototype.isPressed('+key+')')
...
satellite.prototype.clearKey = function(keyIndex) {
  var self = this;
	...
	if (self.socket !== undefined) {
		...
		var buf = protocol.SCMD_DRAW7272_PARSER.serialize({ deviceId: self.deviceId, keyIndex: keyIndex, image: imageBuffer });
		protocol.sendPacket(self.socket, protocol.SCMD_DRAW7272, buf);
	} else {
...
satellite.prototype.clearAllKeys = function() {
  ...
  for (var i = 0; i < 15; ++i) {
		...
		if (self.socket !== undefined) {
			var buf = protocol.SCMD_DRAW7272_PARSER.serialize({ deviceId: self.deviceId, keyIndex: i, image: imageBuffer });
			protocol.sendPacket(self.socket, protocol.SCMD_DRAW7272, buf);
		} else {
			debug('trying to emit to nonexistaant socket: ', self.id);
		}
...
const { prepareButtonBuffer } = require('./usb/common');
satellite.prototype.handleBuffer = function (buffer) {
	prepareButtonBuffer(buffer, this.config.rotation);
};
