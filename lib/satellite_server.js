...
function satelliteServer(_system, _elgatoDM) {
...
satelliteServer.prototype.handleButton = function(packet, socket) {
	...
	if (self.devices[obj.deviceId] !== undefined) {
		self.system.emit(self.devices[obj.deviceId].id + '_button', obj.keyIndex, obj.state);
	}
};
...
satelliteServer.prototype.newDevice = function(packet, socket) {
	...
	id = 'satellite-' + data.serialNumber;
	...
	elgatoDM.addDevice({ path: id }, 'satellite_device');
